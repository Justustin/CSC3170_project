...
The Library Management System employs a modern \textbf{client-server architecture} with a React frontend and Node.js/Express backend, ensuring a clear separation of concerns, enhancing maintainability and scalability.
...
\begin{itemize}
    ...
    \item \textbf{Supabase:} PostgreSQL database with built-in authentication and real-time features.
    \item \textbf{bcrypt:} Library for hashing passwords securely.
    \item \textbf{jsonwebtoken:} Library for JWT token generation and verification.
    \item \textbf{dayjs:} Lightweight date manipulation library.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{React Router:} Client-side routing for single-page application.
    \item \textbf{Axios:} HTTP client for API requests.
    \item \textbf{Lucide React:} Icon library for UI elements.
\end{itemize}

\subsubsection{Key Modules and Functions}

The frontend is organized into role-based page components:

\begin{itemize}
    \item \textbf{Authentication:} Login and Register pages handle user authentication.
    \item \textbf{Patron Pages:} SearchCatalog, BorrowedBooks, BorrowHistory, Notifications, Profile.
    \item \textbf{Librarian Pages:} ManageResources, ManageUsers, Borrowings, Overdue, Reports, LibraryLogs.
    \item \textbf{Director Pages:} DirectorDashboard with system statistics and librarian management.
\end{itemize}
